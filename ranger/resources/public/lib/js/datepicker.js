      ...
      var ids = {},
        ...
        fill = function(el) {
          ...
          for (var i = 0; i < options.calendars; i++) {
            ...
            while (cnt < 42) {
              ...
              if (
                options.date &&
                (!$.isArray(options.date) || options.date.length > 0)
              ) {
                // update to fill to allow base and compare ranges
                if (
                  options.date.length > 2 &&
                  (val >= options.date[2] && val <= options.date[3])
                ) {
                  //console.log(data.weeks[indic].days[indic2]);
                  var dateCandidate = data.weeks[indic].days[indic2];

                  var containsDisabled = _.contains(
                    dateCandidate.classname,
                    "datepickerDisabled"
                  );

                  if (containsDisabled === false)
                    dateCandidate.classname.push("datepickerCompareSelected");
                } else if (
                  fromUser.selected ||
                  options.date == val ||
              ...
              data.weeks[indic].days[indic2].classname = data.weeks[indic].days[
                indic2
              ].classname.join(" ");
              cnt++;
              date.addDays(1);
        ...
        layout = function(el) {
          ...
          if (options.extraHeight === false) {
            ...
            options.extraHeight =
              divs.get(0).offsetHeight + divs.get(1).offsetHeight; // heights from top/bottom borders
            options.extraWidth =
              divs.get(2).offsetWidth + divs.get(3).offsetWidth; // widths from left/right borders
          }
            ...
            if (parentEl.is("th")) {
              ...
              if (el.hasClass("datepickerMonth")) {
                ...
                if (options.mode == "range") {
                  ...
                  if (tblEl.eq(0).hasClass("datepickerViewDays")) {
                    tblEl
                      .eq(0)
                      .toggleClass("datepickerViewDays datepickerViewMonths");
                    el.find("span").text(tmp.getFullYear());
                  } else if (tblEl.eq(0).hasClass("datepickerViewMonths")) {
                    tblEl
                      .eq(0)
                      .toggleClass("datepickerViewMonths datepickerViewYears");
                    el.find("span").text(
                    ...
                    tblEl
                      .eq(0)
                      .toggleClass("datepickerViewYears datepickerViewDays");
                    el.find("span").text(
                      tmp.getMonthName(true) + ", " + tmp.getFullYear()
                    );
                  }
                }
              } else if (
                ...
                if (tblEl.eq(0).hasClass("datepickerViewDays")) {
                  options.current.addMonths(
                    el.hasClass("datepickerGoPrev") ? -1 : 1
                  );
                } else if (tblEl.eq(0).hasClass("datepickerViewMonths")) {
                  ...
                  options.current.addYears(
                    el.hasClass("datepickerGoPrev") ? -12 : 12
                  );
                }
                fillIt = true;
              }
              ...
              if (tblEl.eq(0).hasClass("datepickerViewMonths")) {
                ...
                options.current.setFullYear(
                  parseInt(
                    tblEl.find("thead th a.datepickerMonth span").text(),
                    10
                  )
                );
                ...
                tblEl
                  .eq(0)
                  .toggleClass("datepickerViewMonths datepickerViewDays");
              } else if (tblEl.eq(0).hasClass("datepickerViewYears")) {
                // clicked a year cell
                options.current.setFullYear(parseInt(el.text(), 10));
                tblEl
                  .eq(0)
                  .toggleClass("datepickerViewYears datepickerViewMonths");
              } else {
                // clicked a day cell
        ...
        hide = function(ev) {
          if (
            ...
            if (
              ev.data.cal
                .data("datepicker")
                .onBeforeHide.apply(this, [ev.data.cal.get(0)]) != false
            ) {
              ev.data.cal.hide();
              ev.data.cal
                .data("datepicker")
                .onAfterHide.apply(this, [ev.data.cal.get(0)]);
            ...
            if (mode != "single") {
              if (!$.isArray(date)) {
                ...
                if (mode == "range") {
                  ...
                  date.push(
                    new Date(date[0]).setHours(23, 59, 59, 0).valueOf()
                  );
                }
              } else {
